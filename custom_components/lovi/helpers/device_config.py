...
import custom_components.lovi.devices as config_dir
...
class TuyaDeviceConfig:
    ...
    def __init__(self, fname):
        ...
        _CONFIG_DIR = _device_config_dir()
        if _CONFIG_DIR is None:
            raise RuntimeError("Unable to determine config directory for devices")
        self._fname = fname
...
def _device_config_dir():
    """Return the directory path for device configs."""
    if hasattr(config_dir, '__path__') and config_dir.__path__:
        return config_dir.__path__[0]
    if config_dir.__file__:
...
def available_configs():
    ...
    _CONFIG_DIR = _device_config_dir()
    if _CONFIG_DIR is None:
        _LOGGER.error("Unable to determine config directory for devices")
        return

    for direntry in scandir(_CONFIG_DIR):
        if (
            direntry.is_file()
            and not direntry.name.startswith("._")
